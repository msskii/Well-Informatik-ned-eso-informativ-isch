...
Building::Building(int x, int y, uint16_t buildingNumber): xcoord(x), ycoord(y), data({buildingNumber})
{
...
void Building::render(SDL_Renderer *renderer, int xoffset, int yoffset)
{
    ...
    SDL_Rect src = {0, 0, data.sizeX * TILE_SIZE / 2 + 1, data.sizeY * TILE_SIZE / 2 + 1}; // For individual 32 by 32 tiles
