...
static const char* updateDebugText(Menu *menu, DebugText *text)
{
    ...
    Player *player = menu->window->level->getLocalPlayer(); // Single player only, no coordinates necessary
    if(text->y == 000) snprintf(text->buffer, text->maxLength + 1, text->format, player->data.x_pos);
    else if(text->y == 100) snprintf(text->buffer, text->maxLength + 1, text->format, player->data.y_pos);
    else if(text->y == 200) snprintf(text->buffer, text->maxLength + 1, text->format, player->_z);
